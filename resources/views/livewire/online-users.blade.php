...
<div wire:poll.2000ms="getOnlineUsers" class="p-6 text-white">
    ...
    @else
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            @foreach($directories as $page => $data)
                @php
                    $iconColorClass = $userCounts[$page] > 0 ? 'text-green-400' : 'text-gray-400';
                @endphp
                <div class="bg-gray-800 p-4 rounded-lg flex items-center">
                    <i class="{{ $data['icon'] }} {{ $iconColorClass }} mr-4 p-2 text-3xl"></i>
                    <div>
                        <span class="text-lg font-bold">{{ $userCounts[$page] }}</span>
                        <span class="block text-sm text-gray-300">{{ $data['label'] }}</span>
                    </div>
                </div>
            @endforeach
        </div>
